...
export default function Add() {
  ...
  useEffect(async () => {
    if (path.updateData && !update) {
      ...
      var buffer = new Buffer.from(path.updateData.data.img, "base64");
      ...
      const resizedImage = await imageResize(blob, 300, 350);
  ...
  const handleChange = async (e) => {
    ...
    setData({ ...data, img: e.target.files[0], image: e.target.files[0] });
  ...
  const handleClick = async () => {
    ...
    if (update) {
      ...
      if (data.image !== "") {
        ...
        console.log(data);
        await axios
          .put(`http://localhost:3001/${user._id}/file`, formdata)
          .then((res) => {
            console.log(res);
            ...
            updatedPlaces.forEach((element) => {
              if (element._id === path.updateData.data._id) {
                ...
                element.image_path = res.data.image_path;
                element.img = res.data.img;
      ...
      if (data.lat == 23.246816) {
        ...
        await axios
          .post(`http://localhost:3001/${user._id}`, formdata)
          .then((res) => {
            ...
            <div className="addr1_img">
              <div className="addr1_img_block">
                {image ? (
                  ...
                  <BsFillImageFill color="white" size="300" />
                )}
              </div>
            ...
            <div className="addr1_info">
              <div className="addr1_info_text">
                ...
                <textarea
                  ...
                  required={true}
                  onChange={(e) =>
                    setData({ ...data, location: e.target.value.trim() })
                  }
                />
                <h5>About</h5>
                <textarea
                  ...
                  rows="10"
                  onChange={(e) =>
                    setData({ ...data, about: e.target.value.trim() })
                  }
                />
              </div>
            </div>
